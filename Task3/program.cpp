...
int main() {
    ...
    std::cout << "The list has now " << list.getCount() << " elements and a capacity of " << list.getCapacity() <<
    std::endl;

    list.reserve(10);

    std::cout << "Created object back, with the name \"" << list.createObject_back("blub")->GetName() << "\"" <<
    std::endl;
    std::cout << "Created object back, with the name \"" << list.createObject_back("inner cookie")->GetName() << "\"" <<
    std::endl;
    std::cout << "Created object back, with the name \"" << list.createObject_back("back cookie")->GetName() << "\"" <<
    std::endl;
